...
class FeatureExtractor:
    ...
    def extract_features(self, audio_path):
        ...
        try:
            ...
            if len(y) > sr * 3:  y = y[(len(y) - sr * 3) // 2: (len(y) - sr * 3) // 2 + sr * 3]
            if len(y) == 0:
            ...
            features.append(np.mean(zcr))  # 4 维

            # 2. 频域 20 维（一次性 try，失败整包填 0）
            try:
                mfcc = librosa.feature.mfcc(y=y, sr=sr, n_mfcc=13)
                mfcc_mean = np.mean(mfcc, axis=1)  # 13 维
                ...
                features.append(np.mean(rolloff))  # 1 维

                cent = librosa.feature.spectral_centroid(y=y, sr=sr)
                bw = librosa.feature.spectral_bandwidth(y=y, sr=sr)
                features.append(np.mean(cent))
                features.append(np.mean(bw))  # 2 维
                # 13 + 4 + 1 + 2 = 20 维
            except Exception as e:
                print(f"频域提取失败 {audio_path}: {e}")
                features.extend([0] * 20)  # 统一 20 维 0

            # 3. 基频 2 维
            pitch_features = self.extract_pitch_features(y, sr)
            features.extend(pitch_features)  # 2 维

            # 4. MFCC 统计量 6 维（补齐到 32）
            features.append(np.max(mfcc))  # 1
            features.append(np.std(mfcc))  # 1
            features.append(np.min(mfcc))  # 1
            from scipy import stats
            features.append(stats.skew(mfcc, axis=None))  # 1
            features.append(stats.kurtosis(mfcc, axis=None))  # 1
            features.append(np.mean(np.ptp(mfcc, axis=1)))  # 1（MFCC 峰-峰均值）
...
def main():
    ...
    data = preprocessor.load_and_analyze_data(r"D:\专用轻量分类器\jsonl_dataset\train_smart.jsonl")
